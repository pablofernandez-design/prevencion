...
(function () {
  ...
  function infoHTML(){
    const enlaces = ENLACES.map(e=>`<div class="area-link"><span class="area-link-t">${esc(e)}</span><a href="javascript:void(0)" class="area-link-cta">Acceder →</a></div>`).join('');
    const fichas = FICHAS.map(([t,d])=>`<div class="area-ficha"><span class="area-ficha-t">${esc(t)}</span><span class="area-ficha-d">${esc(d)}</span><a href="javascript:void(0)" class="area-ficha-btn">Abrir ficha →</a></div>`).join('');
  ...
  function tenHTML(){
    return `<ul class="area-checks">${TENCUENTA.map((i,ix)=>`<li><span class="area-badge">${ix<2?TEN_TICK:TEN_CHECK}</span><span>${esc(i)}</span></li>`).join('')}</ul>`;
  }
  function omsHTML(){
    ...
    host.innerHTML = `<article class="area-detail">
      ...
      <div class="area-body">
        ...
        <div class="area-doc">
          ...
          <section id="a-oms" class="area-card"><h2 class="area-h2">Recomendaciones de salud de la OMS</h2><p class="area-p">Según la <b>Organización Mundial de la Salud (OMS)</b>, los beneficios de realizar actividad física regular son múltiples:</p>${omsHTML()}<div class="area-oms-box"><span class="area-lbl">Recomendación de la OMS</span><p>${esc(OMS_REC)}</p></div></section>
        </div>
      </div>
    </article>`;
