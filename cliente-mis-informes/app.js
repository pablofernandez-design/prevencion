  ...
  function pautasHTML(slug){
    ...
    if(p.type==='cols'){
      ...
      html += `<div class="area-pnum">${p.items.map((it,i)=>{
        ...
        const bl = (it && it.bullets) ? `<ul>${it.bullets.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>` : '';
        return `<details class="area-pnum-item"><summary><span class="area-pnum-n">${i+1}</span><span class="area-pnum-t">${esc(t)}</span><svg class="area-pnum-chev" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></summary><div class="area-pnum-body">${body}${bl}</div></details>`;
      }).join('')}</div>`;
      if(p.table){
